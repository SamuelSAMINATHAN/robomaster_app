...
export default function SimpleBlockly() {
  ...
  useEffect(() => {
    ...
    // Fonction pour charger Blockly depuis CDN
    const loadBlockly = async () => {
      try {
        // Vérifier si Blockly est déjà chargé
        if (window.Blockly) {
          console.log("Blockly est déjà chargé");
          initBlockly();
          return;
        }

        console.log("Chargement de Blockly depuis CDN...");
        
        // Charger Blockly depuis CDN
        const script = document.createElement('script');
        script.src = 'https://unpkg.com/blockly/blockly_compressed.js';
        script.async = true;
        
        script.onload = () => {
          console.log("Blockly core chargé, chargement des blocs...");
          // Charger les blocs
          const blocksScript = document.createElement('script');
          blocksScript.src = 'https://unpkg.com/blockly/blocks_compressed.js';
          blocksScript.async = true;
          
          blocksScript.onload = () => {
            console.log("Blocs chargés, chargement de Python...");
            // Charger le générateur Python
            const pythonScript = document.createElement('script');
            pythonScript.src = 'https://unpkg.com/blockly/python_compressed.js';
            pythonScript.async = true;
            
            pythonScript.onload = () => {
              console.log("Python chargé, initialisation de Blockly...");
              // Définir les blocs personnalisés après le chargement de Blockly et Python
              defineCustomBlocks();
              initBlockly();
            };
        ...
        script.onerror = () => {
          setError("Impossible de charger Blockly depuis CDN");
        };
        
        document.body.appendChild(script);
      } catch (err) {
        console.error("Erreur lors du chargement de Blockly:", err);
    ...
    const defineCustomBlocks = () => {
      try {
        if (!window.Blockly) {
          console.error("Blockly n'est pas disponible pour définir les blocs personnalisés");
          return;
        }
        ...
        window.Blockly.Blocks['robomaster_init'] = {
          init: function() {
        ...
        window.Blockly.Blocks['robomaster_close'] = {
          init: function() {
            this.appendDummyInput()
        ...
        // Bloc chassis_move
        window.Blockly.Blocks['chassis_move'] = {
          init: function() {
        ...
        // Bloc chassis_stop
        window.Blockly.Blocks['chassis_stop'] = {
          init: function() {
            this.appendDummyInput()
        ...
        window.Blockly.Blocks['chassis_set_speed'] = {
          init: function() {
        ...
        window.Blockly.Blocks['led_set_color'] = {
          init: function() {
        ...
        // Bloc LED effet
        window.Blockly.Blocks['led_effect'] = {
          init: function() {
            this.appendDummyInput()
        ...
        // Blocs pour gimbal
        window.Blockly.Blocks['gimbal_move'] = {
          init: function() {
        ...
        window.Blockly.Blocks['gimbal_stop'] = {
          init: function() {
            this.appendDummyInput()
        ...
        // Bloc pour la caméra
        window.Blockly.Blocks['camera_start'] = {
          init: function() {
            this.appendDummyInput()
        ...
        window.Blockly.Blocks['camera_stop'] = {
          init: function() {
            this.appendDummyInput()
        ...
        window.Blockly.Blocks['robotic_arm_move'] = {
          init: function() {
        ...
        window.Blockly.Blocks['robotic_arm_stop'] = {
          init: function() {
            this.appendDummyInput()
        ...
        window.Blockly.Blocks['battery_level'] = {
          init: function() {
            this.appendDummyInput()
        ...
        window.Blockly.Python['robomaster_init'] = function(block) {
          return 'robot = RoboMaster()\nrobot.initialize()\n';
        };

        // Générateur pour robomaster_close
        window.Blockly.Python['robomaster_close'] = function(block) {
          return 'robot.close()\n';
        };

        // Générateur pour chassis_move
        window.Blockly.Python['chassis_move'] = function(block) {
          const x_speed = window.Blockly.Python.valueToCode(block, 'X_SPEED', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const y_speed = window.Blockly.Python.valueToCode(block, 'Y_SPEED', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const z_speed = window.Blockly.Python.valueToCode(block, 'Z_SPEED', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const duration = window.Blockly.Python.valueToCode(block, 'DURATION', window.Blockly.Python.ORDER_ATOMIC) || '0';
          
          return `robot.chassis.move(${x_speed}, ${y_speed}, ${z_speed}, ${duration})\n`;
        };

        // Générateur pour chassis_stop
        window.Blockly.Python['chassis_stop'] = function(block) {
          return 'robot.chassis.stop()\n';
        };

        // Générateur pour chassis_set_speed
        window.Blockly.Python['chassis_set_speed'] = function(block) {
          const speed = window.Blockly.Python.valueToCode(block, 'SPEED', window.Blockly.Python.ORDER_ATOMIC) || '0';
          return `robot.chassis.set_speed(${speed})\n`;
        };

        // Générateur pour led_set_color
        window.Blockly.Python['led_set_color'] = function(block) {
          const r = window.Blockly.Python.valueToCode(block, 'R', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const g = window.Blockly.Python.valueToCode(block, 'G', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const b = window.Blockly.Python.valueToCode(block, 'B', window.Blockly.Python.ORDER_ATOMIC) || '0';
        ...
        window.Blockly.Python['led_effect'] = function(block) {
          const effect = block.getFieldValue('EFFECT');
          const r = window.Blockly.Python.valueToCode(block, 'R', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const g = window.Blockly.Python.valueToCode(block, 'G', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const b = window.Blockly.Python.valueToCode(block, 'B', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const frequency = window.Blockly.Python.valueToCode(block, 'FREQUENCY', window.Blockly.Python.ORDER_ATOMIC) || '1';
          
          return `robot.led.set_effect("${effect}", ${r}, ${g}, ${b}, ${frequency})\n`;
        };

        // Générateur pour gimbal_move
        window.Blockly.Python['gimbal_move'] = function(block) {
          const pitch = window.Blockly.Python.valueToCode(block, 'PITCH_SPEED', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const yaw = window.Blockly.Python.valueToCode(block, 'YAW_SPEED', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const duration = window.Blockly.Python.valueToCode(block, 'DURATION', window.Blockly.Python.ORDER_ATOMIC) || '0';
        ...
        window.Blockly.Python['gimbal_stop'] = function(block) {
          return 'robot.gimbal.stop()\n';
        };

        // Générateur pour camera_start
        window.Blockly.Python['camera_start'] = function(block) {
          return 'robot.camera.start()\n';
        };

        // Générateur pour camera_stop
        window.Blockly.Python['camera_stop'] = function(block) {
          return 'robot.camera.stop()\n';
        };

        // Générateur pour robotic_arm_move
        window.Blockly.Python['robotic_arm_move'] = function(block) {
          const x = window.Blockly.Python.valueToCode(block, 'X', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const y = window.Blockly.Python.valueToCode(block, 'Y', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const z = window.Blockly.Python.valueToCode(block, 'Z', window.Blockly.Python.ORDER_ATOMIC) || '0';
          const speed = window.Blockly.Python.valueToCode(block, 'SPEED', window.Blockly.Python.ORDER_ATOMIC) || '10';
          
          return `robot.arm.move(${x}, ${y}, ${z}, ${speed})\n`;
        };

        // Générateur pour robotic_arm_stop
        window.Blockly.Python['robotic_arm_stop'] = function(block) {
          return 'robot.arm.stop()\n';
        };

        // Générateur pour battery_level
        window.Blockly.Python['battery_level'] = function(block) {
          return ['robot.battery.get_level()', window.Blockly.Python.ORDER_FUNCTION_CALL];
        };
    ...
    const initBlockly = () => {
      try {
        if (!blocklyContainerRef.current || !window.Blockly) {
          console.error("Référence au conteneur ou Blockly non disponible");
          return;
        }
        ...
        workspace = window.Blockly.inject(blocklyContainerRef.current, {
          toolbox,
          trashcan: true,
        ...
        workspace.addChangeListener(() => {
          try {
            ...
            const code = window.Blockly.Python ? 
              window.Blockly.Python.workspaceToCode(workspace) :
              window.Blockly.JavaScript.workspaceToCode(workspace) + "\n// Python non disponible, code JavaScript généré à la place";
